...
func TokenMessageContextHandler() func(ctx *openwechat.MessageContext) {
	return func(ctx *openwechat.MessageContext) {
		...
		if err != nil {
			logrus.Warning(fmt.Sprintf("init token message handler error: %s", err))
			return
		}
		...
		err = handler.handle()
		if err != nil {
			logrus.Warning(fmt.Sprintf("handle token message error: %s", err))
		}
	}
}
